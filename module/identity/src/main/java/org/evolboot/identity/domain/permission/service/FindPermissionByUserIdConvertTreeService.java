...
public class FindPermissionByUserIdConvertTreeService {
    ...
    public List<Permission> findPermissionByUserIdConvertTree(Long userId) {
        ...
        if (user.hasUserIdentity(UserIdentity.ROLE_SUPER_ADMIN)) {
            return PermissionUtil.convertTree(repository.findByScope(PermissionScope.PLATFORM));
        }
        if (user.hasUserIdentity(UserIdentity.ROLE_TENANT_OWNER)) {
            return PermissionUtil.convertTree(repository.findByScope(PermissionScope.TENANT));
        }
        Set<Long> roleId = user.getRoleId();
        if (ExtendObjects.isEmpty(roleId)) {
            return Collections.emptyList();
        }
        List<Role> roles = roleQueryService.findAllByIdAndTenantId(roleId, user.getTenantId());
        if (roles.isEmpty()) {
            return Collections.emptyList();
        }
        ...
        return PermissionUtil.convertTree(repository.findAll(
                PermissionQueryRequest.builder().ids(collect).build()
        ));
    }
