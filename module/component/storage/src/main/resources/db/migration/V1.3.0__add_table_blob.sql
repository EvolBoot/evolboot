...
(
    ...
    file_type_        tinyint NULL COMMENT '文件类型',
    creator_user_id_  BIGINT NULL COMMENT '创建者用户ID（用于审计）',
    owner_type_       VARCHAR(20)  NOT NULL DEFAULT 'USER' COMMENT '归属类型：USER-个人, TENANT-租户, PLATFORM-平台',
    owner_id_         BIGINT       NOT NULL COMMENT '归属ID（根据owner_type_对应用户ID或租户ID）',


    PRIMARY KEY (id_),
    INDEX idx_blob_owner (owner_type_, owner_id_)
) ENGINE = InnoDB
